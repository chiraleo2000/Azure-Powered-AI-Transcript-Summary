...
COPY app.py app_func.py backend.py ai_summary.py session_manager.py \
     ...
     audio_enhancer.py azure_keyvault_client.py config.py ./
COPY src/ ./src/
COPY static/ ./static/
...
ENV DEBUG=False
# Concurrency / RAM tuning for B3 (4 vCPU / 7 GiB) — keeps in-flight
# audio enhancement under the memory ceiling for 200-500 MB inputs.
